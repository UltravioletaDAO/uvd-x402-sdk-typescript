...
- `OPERATOR_ABI_V3`: `capture`, `void`, `refund`, `FEE_RECEIVER` and the five `*_PRE_ACTION_CONDITION` getters, each selector checked against the factory bytecode recorded on Arc. On a v3 chain, `AdvancedEscrowClient.release` sends `capture(paymentInfo, amount, 0x)`. `refundInEscrow` sends `void(paymentInfo, 0x)`, which takes no amount and returns the whole `capturableAmount`, so the client reads that amount on-chain (`getHash` + `paymentState`) and sends only when the requested amount equals it; otherwise it sends nothing and returns `errorCode` `ESCROW_VOID_AMOUNT_MISMATCH`, or `ESCROW_NOTHING_TO_VOID` when the amount is 0. To return part of an escrow, `release` the part the receiver keeps and `refundInEscrow` the rest. A v3 call to an operator with no code returns `ESCROW_OPERATOR_NOT_DEPLOYED`; `charge` and `refundPostEscrow` return `ESCROW_UNSUPPORTED_ON_GENERATION` without signing or sending.
...
- `src/escrow-preauth.vectors.json` gains `arc_vector`, appended after every other key (their bytes are unchanged). Its nonces are `AuthCaptureEscrow.getHash` answers read on chain 5042, its USDC domain is the token's own `name()` / `version()`, and its signature also verifies with `eth_account`. Derived by `scripts/derive-arc-escrow-preauth-vector.mjs`.
- Measurements: `src/fixtures/arc-escrow-d.rpc.json`, recorded from the public Arc RPCs by `scripts/record-arc-escrow-d.mjs` (blocks 22435548 on Arc, 63681360 on Arc Testnet): factory bytecode, `computeAddress`, the factory's `ESCROW()` / `PROTOCOL_FEE_CONFIG()`, the collector's `authCaptureEscrow()`, code at every address, USDC `name()` / `version()` / `DOMAIN_SEPARATOR()`, `getHash` and `paymentState`.
